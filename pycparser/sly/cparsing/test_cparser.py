...
class TestCParser_base(unittest.TestCase):
    def parse(self, txt: str, filename: str = '') -> Any:
        return self.cparser.parse(txt, filename)

    def setUp(self) -> None:
        self.cparser = _c_parser

    def assert_coord(self, node: Any, line: int, column: int | None = None, file: str | None = None) -> None:
...
class TestCParser_fundamentals(TestCParser_base):
    def get_decl(self, txt: str, index: int = 0) -> list[Any]:
    ...
    def get_decl_init(self, txt: str, index: int = 0) -> Sequence[Any]:
    ...
    def test_FileAST(self) -> None:
        t = self.parse('int a; char c;')
        self.assertIsInstance(t, c_ast.FileAST)
        self.assertEqual(len(t.ext), 2)

        # empty file
        t2 = self.parse('')
        self.assertIsInstance(t2, c_ast.FileAST)
        self.assertEqual(len(t2.ext), 0)

    def test_empty_toplevel_decl(self) -> None:
        code = 'int foo;;'
        t = self.parse(code)
        self.assertIsInstance(t, c_ast.FileAST)
    ...
    def test_initial_semi(self) -> None:
        t = self.parse(';')
    ...
    def test_simple_decls(self) -> None:
    ...
    def test_func_decls_with_array_dim_qualifiers(self) -> None:
    ...
    def test_qualifiers_storage_specifiers(self) -> None:
        def assert_qs(txt: str, index: int, quals: list[str], storage: list[str]) -> None:
        ...
        self.assertIsInstance(pdecl, c_ast.PtrDecl)
        self.assertEqual(pdecl.quals, ['const'])

    def test_atomic_specifier(self) -> None:
    ...
    def test_sizeof(self) -> None:
        ...
        self.assertIsInstance(s1, c_ast.UnaryOp)
        self.assertEqual(s1.op, 'sizeof')
        self.assertIsInstance(s1.expr, c_ast.ID)
    ...
    def test_offsetof(self) -> None:
        def expand_ref(n: Any) -> list[str | Any]:
        ...
        compound = self.parse(e).ext[0].body
        s1 = compound.block_items[0].init
        self.assertIsInstance(s1, c_ast.FuncCall)
        self.assertIsInstance(s1.name, c_ast.ID)
        self.assertEqual(s1.name.name, 'offsetof')
        self.assertIsInstance(s1.args.exprs[0], c_ast.Typename)
        self.assertIsInstance(s1.args.exprs[1], c_ast.ID)
        s3 = compound.block_items[2].init
        self.assertIsInstance(s3.args.exprs[1], c_ast.StructRef)
        self.assertEqual(expand_ref(s3.args.exprs[1]),
            ['StructRef',
                ['StructRef', ['ID', 'p'], ['ID', 'q']],
                ['ID', 'r']])
        s4 = compound.block_items[3].init
        self.assertIsInstance(s4.args.exprs[1], c_ast.ArrayRef)
    ...
    def test_compound_statement(self) -> None:
        e = """
            ...
            """
        compound = self.parse(e).ext[0].body
        self.assertIsInstance(compound, c_ast.Compound)
        self.assert_coord(compound, 2)

    # The C99 compound literal feature
    #
    def test_compound_literals(self) -> None:
        ps1 = self.parse(r'''
            void foo() {
                p = (long long){k};
    ...
    def test_parenthesized_compounds(self) -> None:
        e = self.parse(r'''
        void foo() {
            int a;
            ({});
        ...
        self.assertIsInstance(body[1], c_ast.Compound)
        self.assertEqual(body[1].block_items, None)

        self.assertIsInstance(body[2], c_ast.Compound)
        self.assertEqual(len(body[2].block_items), 1)
        self.assertIsInstance(body[2].block_items[0], c_ast.Constant)

        self.assertIsInstance(body[3], c_ast.Compound)
        self.assertEqual(len(body[3].block_items), 2)
        self.assertIsInstance(body[3].block_items[0], c_ast.Constant)
        self.assertIsInstance(body[3].block_items[1], c_ast.Constant)

        self.assertIsInstance(body[4], c_ast.Decl)
        self.assertEqual(expand_init(body[4].init),
            ['Compound', [['Constant', 'int', '1']]])

        self.assertIsInstance(body[5], c_ast.Decl)
        self.assertEqual(body[5].init, None)

        self.assertIsInstance(body[6], c_ast.Decl)
        self.assertEqual(expand_init(body[6].init),
            ['Compound', [['Decl', 'x'], ['BinaryOp', ['ID', 'x'], '+', ['Constant', 'int', '2']]]])

        self.assertIsInstance(body[7], c_ast.Assignment)
        self.assertIsInstance(body[7].rvalue, c_ast.Compound)
    ...
    def test_enums(self) -> None:
        ...
        self.assertIsInstance(e1_type, c_ast.Enum)
        ...
        self.assertIsInstance(e2_type, c_ast.Enum)
        self.assertEqual(e2_type.name, 'mysize')

        e2_elist = e2_type.values
        self.assertIsInstance(e2_elist, c_ast.EnumeratorList)

        for e2_eval in e2_elist.enumerators:
            self.assertIsInstance(e2_eval, c_ast.Enumerator)
        ...
        self.assertIsInstance(e3_type, c_ast.Enum)
        e3_elist = e3_type.values
        self.assertIsInstance(e3_elist, c_ast.EnumeratorList)

        for e3_eval in e3_elist.enumerators:
            self.assertIsInstance(e3_eval, c_ast.Enumerator)
    ...
    def test_struct_enum(self) -> None:
        s1 = """
            struct Foo {
                enum Bar { A = 1 };
            };
    ...
    def test_struct_with_extra_semis_inside(self) -> None:
        s1 = """
            struct {
                int a;;
            } foo;
    ...
    def test_struct_with_initial_semi(self) -> None:
        s1 = """
            struct {
                ;int a;
            } foo;
    ...
    def test_anonymous_struct_union(self) -> None:
        s1 = """
            union
            {
                union
    ...
    def test_struct_members_namespace(self) -> None:
    ...
    def test_struct_bitfields(self) -> None:
    ...
    def test_struct_empty(self) -> None:
        """
    ...
    def test_tags_namespace(self) -> None:
        ...
        self.assertIsInstance(s3_type, c_ast.Enum)
        self.assertEqual(s3_type.name, 'mytag')

    def test_multi_decls(self) -> None:
        d1 = 'int a, b;'
    ...
    def test_invalid_multiple_types_error(self) -> None:
        bad = [
            'int enum {ab, cd} fubr;',
            'enum kid char brbr;']
    ...
    def test_invalid_typedef_storage_qual_error(self) -> None:
    ...
    def test_duplicate_typedef(self) -> None:
    ...
    def test_decl_named_inits(self) -> None:
        d1 = 'int a = {.k = 16};'
        self.assertEqual(self.get_decl_init(d1),
            [(   [['ID', 'k']],
                 ['Constant', 'int', '16'])])
    ...
    def test_function_definitions(self) -> None:
        def parse_fdef(str_: str) -> Any:
            return self.parse(str_).ext[0]

        def fdef_decl(fdef: Any) -> list[Any]:
        ...
        self.assertEqual(type(f1.body.block_items[0]), c_ast.Return)
        ...
        self.assertEqual(list(map(type, f2.body.block_items)),
            [c_ast.Decl, c_ast.Decl, c_ast.Assignment, c_ast.Return])
    ...
    def test_static_assert(self) -> None:
        f1 = self.parse('''
    ...
    def test_unified_string_literals(self) -> None:
    ...
    def test_unified_wstring_literals(self) -> None:
    ...
    def test_vla(self) -> None:
        ...
        ''')
        self.assertIsInstance(ps2.ext[0].body.block_items[1].type.dim, c_ast.Assignment)
        self.assertIsInstance(ps2.ext[0].body.block_items[2].type.dim, c_ast.ID)
    ...
    def test_pragmacomp_or_statement(self) -> None:
        ...
        '''
        s1_ast = self.parse(s1)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[1], c_ast.For)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[1].stmt, c_ast.Compound)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[1].stmt.block_items[0], c_ast.Pragma)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[1].stmt.block_items[1], c_ast.Assignment)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[2], c_ast.While)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[2].stmt, c_ast.Compound)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[2].stmt.block_items[0], c_ast.Pragma)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[2].stmt.block_items[1], c_ast.Assignment)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[3], c_ast.Label)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[3].stmt, c_ast.Compound)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[3].stmt.block_items[0], c_ast.Pragma)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[3].stmt.block_items[1], c_ast.Assignment)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[4], c_ast.If)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[4].iftrue, c_ast.Compound)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[4].iftrue.block_items[0], c_ast.Pragma)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[4].iftrue.block_items[1], c_ast.Pragma)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[4].iftrue.block_items[2], c_ast.Assignment)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[5], c_ast.Switch)
        self.assertIsInstance(s1_ast.ext[0].body.block_items[5].stmt.stmts[0], c_ast.Compound)
...
class TestCParser_whole_code(TestCParser_base):
    ...
    #
    class IDNameCounter(c_ast.NodeVisitor):
    ...
    #
    class NodeKlassCounter(c_ast.NodeVisitor):
        def __init__(self, node_klass: type):
    ...
    def assert_all_Constants(self, code: Any, constants: list[str]) -> None:
    ...
    def assert_num_ID_refs(self, code: Any, name: str, num: int) -> None:
    ...
    def assert_num_klass_nodes(self, code: Any, klass: type, num: int) -> None:
    ...
    def test_statements(self) -> None:
        ...
        self.assert_num_klass_nodes(ps1, c_ast.If, 3)
        self.assert_num_klass_nodes(ps1, c_ast.Return, 2)
        self.assert_num_klass_nodes(ps1, c_ast.FuncCall, 1) # strcmp
        self.assert_num_klass_nodes(ps1, c_ast.BinaryOp, 7)
        ...
        ps2 = self.parse(s2)
        self.assert_num_klass_nodes(ps2, c_ast.FuncCall, 6)
        self.assert_num_klass_nodes(ps2, c_ast.FuncDef, 1)
        self.assert_num_klass_nodes(ps2, c_ast.For, 1)
        self.assert_num_klass_nodes(ps2, c_ast.While, 1)
        self.assert_num_klass_nodes(ps2, c_ast.StructRef, 10)
        ...
        ps3 = self.parse(s3)
        self.assert_num_klass_nodes(ps3, c_ast.DoWhile, 1)
    ...
    def test_empty_statements(self) -> None:
        ...
        '''
        ps1 = self.parse(s1)
        self.assert_num_klass_nodes(ps1, c_ast.EmptyStatement, 3)
        self.assert_num_klass_nodes(ps1, c_ast.Return, 1)
    ...
    def test_switch_statement(self) -> None:
        def assert_case_node(node: Any, const_value: str) -> None:
            self.assertIsInstance(node, c_ast.Case)
            self.assertIsInstance(node.expr, c_ast.Constant)
            self.assertEqual(node.expr.value, const_value)

        def assert_default_node(node: Any) -> None:
            self.assertIsInstance(node, c_ast.Default)
    ...
    def test_for_statement(self) -> None:
        ...
        '''
        ps3 = self.parse(s3)
        self.assert_num_klass_nodes(ps3, c_ast.For, 1)
    ...
    def _open_c_file(self, name: str) -> io.TextIOWrapper:
    ...
    def test_whole_file(self) -> None:
        ...
        self.assert_num_klass_nodes(p, c_ast.FuncDef, 5)

        # each FuncDef also has a FuncDecl. 4 declarations
        # + 5 definitions, overall 9
        self.assert_num_klass_nodes(p, c_ast.FuncDecl, 9)

        self.assert_num_klass_nodes(p, c_ast.Typedef, 4)
    ...
    def test_whole_file_with_stdio(self) -> None:
...
class TestCParser_typenames(TestCParser_base):
    ...
    """
    def test_innerscope_typedef(self) -> None:
        ...
        s2 = r'''
            ...
            '''
        self.assertIsInstance(self.parse(s2), c_ast.FileAST)

    def test_ambiguous_parameters(self) -> None:
    ...
    def test_innerscope_reuse_typedef_name(self) -> None:
    ...
    def test_parameter_reuse_typedef_name(self) -> None:
    ...
    def test_nested_function_decls(self) -> None:
        ...
        s1 = r'''
            ...
            '''
        self.assertIsInstance(self.parse(s1), c_ast.FileAST)

    def test_samescope_reuse_name(self) -> None:
