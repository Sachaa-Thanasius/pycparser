...
def get_decl(source: str, index: int = 0) -> list[Any]:
    ...
    FileAST holds a list of 'external declarations'. index is the offset of the desired declaration in that list.
    """

    node = parse(source).ext[index]
    return expand_decl(node)


def get_decl_init(source: str, index: int = 0) -> Sequence[Any]:
    """Returns the expanded initializer of the declaration at index."""
