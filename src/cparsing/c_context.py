...
class CContext:
    """High-level class that wraps the lexing/parsing process, providing shared state and error handling."""
    ...
    def ast(self) -> "c_ast.File":
        """c_ast.File: The AST generated by parsing a given source with `parse()`."""
    ...
    def error(self, message: str, location: Optional[Coord], *args: object) -> NoReturn:
        if location is not None and location.filename != self.filename:
...
def parse(source: str, filename: str = "<unknown>", *, parser_type: type[CParser] = CParser) -> "c_ast.File":
    """Parse a string with C code and create an AST from it.
    ...
    filename: str, default="<unknown>"
        The filename of the source if it has one. Optional.
    parser_type: type[CParser], default=CParser
        The parser class to use when parsing the tokens generated from the file with a lexer. Defaults to the package's
        parser.
    ...
    ctx = CContext(filename, parser_type)
    ctx.parse(source)
    return ctx.ast
...
def parse_file(
    ...
    *,
    ...
    """Parse a file with C code and create an AST from it.
    ...
    use_cpp: bool, default=False
        Whether to attempt to pre-process the file with a local C++ compiler.
    cpp_path: str, default="cpp"
        The path to the C++ compiler. The default assums "cpp" is already on PATH.
    cpp_args: Sequence[str], default=()
        A sequence of command line arguments for cpp, e.g. [r"-I../utils/fake_libc_include"]. Default is an empty
        tuple. Raw strings are recommended, especially when passing in paths.
    parser_type: type[CParser], default=CParser
